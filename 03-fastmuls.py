...
# faster matmuls using custom cuda kernls
...
class MHA(nn.Module):
    ...
    def forward(self, x, mask, freqs_cos, freqs_sin, start_idx=0):
        ...
        Q = matmul_module.matmul(x, self.wq.T)  # [seq_len, 2048]
        K = matmul_module.matmul(x, self.wk.T)  # [seq_len, 2048]
        V = matmul_module.matmul(x, self.wv.T)  # [seq_len, 2048]
        ...
        return matmul_module.matmul(output, self.wo.T)  # [seq_len, 2048]
...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        #     self.w2.T
        # )
        hidden = F.silu(matmul_module.matmul(x, self.w1.T)) * matmul_module.matmul(x, self.w3.T)
        return matmul_module.matmul(hidden, self.w2.T)
...
class Transformer(nn.Module):
    ...
    def forward(self, tokens, start_idx=0):
        ...
        logits = matmul_module.matmul(x, self.output_weights.T).float()
