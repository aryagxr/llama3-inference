...
class MHA(nn.Module):
    def __init__(self, wq, wk, wv, wo, n_heads=config.N_HEADS, n_kv_heads=config.N_KV_HEADS):
        super().__init__()
        #fusing Q/K/V projections into a single weight matrix for one matmul
    ...
    def forward(self, x, mask, freqs_cos, freqs_sin, start_idx=0):
        ...
        #fused QKV projection for new tokens
        x = x.to(self.wqkv.dtype)
        qkv = torch.matmul(x, self.wqkv.T)  # [seq_len, q+k+v]
        Q, K, V = torch.split(
            qkv,
        ...
        #repeat for GQA (use cached values)
...
class FeedForward(nn.Module):
    def __init__(self, w1, w3, w2):
        super().__init__()
        # Fuse the two FFN projections (w1, w3) into a single matmul
...
# Run without profiler to reduce memory pressure. Re-enable profiling if needed.
